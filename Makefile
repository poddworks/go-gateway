...
darwin-static: config
	env CGO_ENABLED=0 GOOS=darwin go build -a -installsuffix cgo -o go-gateway-util-darwin-x86_64 ./cmd-util
	env CGO_ENABLED=0 GOOS=darwin go build -a -installsuffix cgo -o go-gateway-darwin-x86_64 ./cmd

darwin: config
	env GOOS=darwin go build -o go-gateway-util-darwin ./cmd-util
	env GOOS=darwin go build -o go-gateway-darwin ./cmd

linux-static: config
	env CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o go-gateway-util-linux-x86_64 ./cmd-util
	env CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o go-gateway-linux-x86_64 ./cmd

linux: config
	env GOOS=linux go build -o go-gateway-util-linux ./cmd-util
